...
- Art in `Data/Interface/HUDWidgets/lilith/`, two styles, both optional:
  ...
  - **Drain states are separate art.** Add `_drain` and `_death` variants (`energy3_drain.dds`, `bar_fill_death.dds`, also `bar_bg_*` / `bar_frame_*` if wanted) and they are shown exactly as painted while draining / draining to death. Any state without its own file falls back to the base art with a colour multiply (lilac / pink / red); "Tint stage frames by drain state" turns that fallback tint off for stage paintings. Templates for all three states ship.
  - Any DDS works: BC1/BC3/BC7 or uncompressed, any size. Block-compressed art whose width or height is not a multiple of 4 (Direct3D would refuse it) is padded to the next multiple by the widget and drawn cropped, so a 1024x250 BC7 export loads as-is. When a texture is still refused, LilithWidget.log says why.
