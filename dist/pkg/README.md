...
- Art in `Data/Interface/HUDWidgets/lilith/`, two styles, both optional:
  ...
  - Both styles change colour with drain state (lilac idle, pink draining, red drain to death) by a colour multiply, so keep the parts you want to recolour light. "Tint stage frames by drain state" can be turned off to keep a stage painting's own colours; the crop style's `bar_fill.dds` is always tinted.
